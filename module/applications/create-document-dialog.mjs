...
export default class CreateDocumentDialog extends Dialog5e {
  ...
  static PARTS = {
    ...super.PARTS,
    content: {
      template: "systems/dragons-and-ballz/templates/apps/document-create.hbs"
    }
  };
  ...
  async _prepareContentContext(context, options) {
    ...
    let defaultType = this.options.createData.type
      ?? game.settings.get("dragons-and-ballz", "defaultDocumentSubtypes")[this.documentName];
  ...
  static async #handleFormSubmission(event, form, formData) {
    ...
    this.#submitted = true;
    const subtypes = game.settings.get("dragons-and-ballz", "defaultDocumentSubtypes");
    subtypes[this.documentName] = this.options.createData.type;
    game.settings.set("dragons-and-ballz", "defaultDocumentSubtypes", subtypes);
    await this.close();
  }
